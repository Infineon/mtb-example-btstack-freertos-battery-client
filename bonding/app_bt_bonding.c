...
*              library for the BTSTACK Battery Client Example for ModusToolbox.
*
...
{
    cy_rslt_t rslt = mtb_kvstore_write(&kvstore_obj, "host_info", (uint8_t *)&battery_client_hostinfo, sizeof(host_info_t));
    return rslt;
}
...
cy_rslt_t app_bt_read_hostinfo(void)
{
    ...
    cy_rslt_t rslt = mtb_kvstore_read(&kvstore_obj, "host_info", (uint8_t *)&battery_client_hostinfo, &data_size);
    return rslt;
}
